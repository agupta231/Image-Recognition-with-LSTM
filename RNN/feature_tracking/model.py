...
last_normalized = 3 * tf.div(tf.sub(last, tf.reduce_min(last)), tf.sub(tf.reduce_max(last), tf.reduce_min(last))) - 1.5
...
final_dropout = tf.nn.dropout(dc3_out, keep_prob=dropout)

# Create softmax Layer
softmax_w = weight_variable([DEEP_CON_3, OUTPUT_SIZE])
